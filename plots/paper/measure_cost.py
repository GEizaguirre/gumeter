...
if __name__ == "__main__":
            ...
            if backend_name != "aws_batch":
                ...
                for file_name in matching_files:
                    ...
                    if os.path.isfile(log_file):
                        ...
                        elasticity = get_cost(backend_data, backend_name)
    ...
    ax.set_ylabel("Execution time\n(s, mean ± std)", fontsize=10)
    ax.set_xticks(x + width * (len(backends_list) - 1) / 2)
    ax.set_xticklabels(
        ["Monte Carlo \nStock", "Monte Carlo \nPi", "Terasort", "Mandelbrot"],
        fontsize=10,
    )
    ax.tick_params(axis="y", labelsize=12)
    ax.set_axisbelow(True)
    ax.grid(True, axis="y", linestyle="--", alpha=0.7)
    # y ticks round to 2 decimals and add $ sign
    ax.yaxis.set_major_formatter("${x:,.2f}")
    # each 0.02
    ax.yaxis.set_major_locator(plt.MultipleLocator(0.02))

    # no margin
    plt.subplots_adjust(left=0.21, right=0.99, top=0.99, bottom=0.22)
